...
exports.run = function (message, page, bot, db, thing) {
    ...
    db.query("SELECT username, " + thing + " AS thing FROM members WHERE server = ? AND "
        + thing + " > 0 AND active=1 ORDER BY " + thing + " DESC",
        [message.channel.guild.id], function (err, rows) {

        if (rows.length < 1) {
            return message.channel.send('No one has any ' + thing + ' currently.');
        }
        ...
        rows.forEach(function (member) {
            ...
            entries.push(rank + ": " + member.username + " - " + member.thing + " " + thing,);
        });

        const embeds = [];
        let i, j, p, tempArray, chunk = 20;
        for (i = 0, p = 1, j = entries.length; i < j; i += chunk, p++) {
            tempArray = entries.slice(i, i + chunk);
            embeds.push(
                new MessageEmbed()
                    .setTitle(":first_place: Ranking for " + thing)
