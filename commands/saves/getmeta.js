...
exports.run = function (message, keyword, bot, db) {
  ...
  db.query(
    "SELECT keyword, value, uses, saver.username AS username, lastused, approved, timeadded \
              FROM data_store \
    ...
    function (err, rows) {
      if (rows[0] == null) {
        ...
        let lastused = rows[0].lastused !== null ? rows[0].lastused.toString() : "never";
        let timeadded =
          rows[0].timeadded !== null ? rows[0].timeadded.toString() : "a long time ago";
        let fieldList = [
